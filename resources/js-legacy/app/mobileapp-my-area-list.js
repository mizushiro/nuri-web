...
(function($, window, document){
	...
	MyAreaList.prototype = {
		...
		updateBookmarks: function() {
			var self = this;
			if(!self.config.bookmarks) return;
			var $wrapper = $('#' + self.id);
			...
			template += '</label>';
			template += '</li>{{/data}}';
			var data = self.config.bookmarks.slice((self.state.page - 1)*PAGE_SIZE, self.state.page*PAGE_SIZE);
		...
		updateRecent: function() {
			...
			template += '</label>';
			template += '</li>{{/data}}';
			var data = self.config.recent.slice(0, 3);
			for(var i = 0 ; i < data.length ; i++) {
				data[i].index = i;
			}
			var html =  Mustache.render(template, {data: data});
		...
		updatePaging: function() {
			var self = this;
			var $wrapper = $('#' + self.id);
			var template = '{{#data}}<button class="num {{#selected}}on{{/selected}}" data-page="{{page}}">{{page}}</button>{{/data}}';
			...
			var html = Mustache.render(template, {data: data});
			$wrapper.find('div[data-role="paging"]').html('<button class="page-prev">&nbsp;</button>' + html + '<button class="page-next">&nbsp;</button>');
		},
		...
		addEventHandler: function() {
			...
			$wrapper.on('click', 'a[data-role="delete-bookmark"]', function(e) {
				...
				self.updateBookmarks();
				$wrapper.find('button[data-role="save-dong"]').first().trigger('focus');
			});
