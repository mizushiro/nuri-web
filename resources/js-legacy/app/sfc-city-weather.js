...
(function($, window, document){
	var VER="0.1", 
	SfcCityWeather = function(wrapperId) {
	...
	SfcCityWeather.prototype = {
		...
		getData: function(dtm) {
			...
			return $.ajax({
				url: prefix + "wnuri-fct/weather/sfc-city-weather.do",
				data: data,
				dataType: "html"
			});
		},
		...
		addEventHandler: function() {
			...
			$wrapper.on('change', 'select[name="reg"]', function(e) {
				// 접근성(의도하지 않은 동작) 
				//self.updateData();
			});
			...
			$wrapper.on('keyup', 'input[name="tm"]', function(e) {
				if (e.keyCode == 13) {
					self.updateData();
		        }
			});
			$wrapper.on('click', 'a[data-role="view-city"]', function(e) {
				var dataType = $(this).attr('data-type');
				var dataTm = $(this).attr('data-tm');
				var dataReg = $(this).attr('data-reg');
				var dataStn = $(this).attr('data-stn');
				$wrapper.find('input[name="stn"]').val(dataStn);
				self.updateData();
			});
		}
	};
	
	if (typeof exports !== 'undefined') exports.SfcCityWeather = SfcCityWeather;
	else window.SfcCityWeather = SfcCityWeather;
