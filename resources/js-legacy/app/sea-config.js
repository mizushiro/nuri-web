...
(function($, window, document){
	...
	SeaConfig.prototype = {
		...
		updateView: function(type, data) {
			...
			var template = '{{#data}}<li><div class="radio-wrap"><button id="rdo_sea_config_{{type}}{{index}}" class="addr-chk-btn rdo-g {{#checked}}on{{/checked}}" name="{{type}}Area" value="{{code}}" data-name="{{name}}">{{name}}</button></div></li>{{/data}}';
		...
		updateBookmarks: function() {
			...
			var template = '{{#data}}';
			template += '<div class="accordion-dropdown-wrap"><button type="button" class="co-btn" ';
			...
			template += '>{{fullName}}</button><a href="#delete-bookmark" class="item-hide-btn delete-bookmark"><span class="hid">{{fullName}} 삭제</span></a></div>';
			template += '{{/data}}';
		...
		addEventHandler: function() {
			...
			$wrapper.find('button[data-role="save-area"]').on('click', function(e) {
				e.preventDefault();
			...
			$wrapper.find('button[data-role="view-area"]').on('click', function(e) {
				e.preventDefault();
			...
			// 최상위 바다예보구역 클릭
			$wrapper.on('click', 'button[name="topArea"]', function(e) {
				$wrapper.find('button[name="topArea"]').removeClass('on');
				$(this).addClass('on');
				var code = $(this).val();
			...
			// 중간 바다예보구역 클릭
			$wrapper.on('click', 'button[name="midArea"]', function(e) {
				$wrapper.find('button[name="midArea"]').removeClass('on');
				$(this).addClass('on');
				var code = $(this).val();
				var name = $(this).attr('data-name');
			...
			// 중간 바다예보구역 클릭
			$wrapper.on('click', 'button[name="btmArea"]', function(e) {
				$wrapper.find('button[name="btmArea"]').removeClass('on');
				$(this).addClass('on');
				var code = $(this).val();
				var name = $(this).attr('data-name');
