	...
	DongConfig.prototype = {
		...
		writeConfig: function() {
			if(this.config) {
				...
				for(var i = 0 ; i < this.config.bookmarks.length ; i++) {
					createFullName(this.config.bookmarks[i]);
					this.config.bookmarks[i].dong.code_type = this.config.bookmarks[i].dong.code.length == 12 ? "01":"02";
					this.config.bookmarks[i].code_type = this.config.bookmarks[i].dong.code_type;
					this.config.bookmarks[i].dong.wide_name = this.config.bookmarks[i].wide.name;
					this.config.bookmarks[i].dong.city_name = this.config.bookmarks[i].city.name;
				}
				if(this.config.selectedBookmark) {
					createFullName(this.config.selectedBookmark);
					this.config.selectedBookmark.dong.code_type = this.config.selectedBookmark.dong.code.length == 12 ? "01":"02";
					this.config.selectedBookmark.code_type = this.config.selectedBookmark.dong.code_type;
					this.config.selectedBookmark.dong.wide_name = this.config.selectedBookmark.wide.name;
					this.config.selectedBookmark.dong.city_name = this.config.selectedBookmark.city.name;	
				}
				...
				window.parent.postMessage({
					...
					selectedBookmark: this.config.selectedBookmark,
				}, '*');
			}
		},
		...
		onAddAreaRecent: function(recent) {
			GlobalEvent.trigger($.Event("onAddAreaRecent"), [recent, this]);
		},
		...
		addBookmark: function() {
			...
			if(!base.state.isFullySelected()) {
				if(base.searchType == SEARCH_TYPE.LOAD) {
					alert("도로명까지 선택해 선택해 주세요.");
				} else {
					alert("읍/면/동까지 선택해 선택해 주세요.");	
				}
				return null;
			}
		...
		updateView: function(type, data, noFocus) {
			...
			var $wrapper = $('#' + base.id);
			var $view = $wrapper.find('ul[data-role="result-holder"]');
			var template = '{{#data}}<li><div class="radio-wrap"><button type="button" class="addr-chk-btn" data-code="{{code}}" data-name="{{name}}" data-x="{{x}}" data-y="{{y}}" data-lat="{{lat}}" data-lon="{{lon}}" data-level="{{level}}">{{name}}</button></div></li>{{/data}}';
			...
			var upButton = '<li><div class="radio-wrap"><button type="button" class="addr-chk-btn" data-goto="' + (type==TYPE.CITY?TYPE.WIDE:(type==TYPE.DONG?TYPE.CITY:""))+ '">↑ 상위</button></li>';
			if(type == TYPE.WIDE) {
				$view.html(html);
			} else {
			...
			if(!noFocus) $view.find('button.addr-chk-btn').first().trigger('focus');
		},
		...
		updateSelectedView: function() {
			...
			if(selected.wide && selected.wide.code) {
				$wrapper.find('button[data-role="selected-wide"]').html(selected.wide.name);	
			} else {
				$wrapper.find('button[data-role="selected-wide"]').html("시/도 선택");
			}
			
			if(selected.city && selected.city.code) {
				$wrapper.find('button[data-role="selected-city"]').html(selected.city.name);	
			} else {
				$wrapper.find('button[data-role="selected-city"]').html("구/군 선택");
			}
			
			if(selected.dong && selected.dong.code) {
				$wrapper.find('button[data-role="selected-dong"]').html(selected.dong.name);	
			} else {
				if(base.searchType == SEARCH_TYPE.LOAD) {
					$wrapper.find('button[data-role="selected-dong"]').html("도로명 선택");	
				} else {
					$wrapper.find('button[data-role="selected-dong"]').html("읍/면/동 선택");	
				}
			}
		},
		...
		addEventHandler: function() {
			...
			$wrapper.find('button[data-role="save-dong"]').on('click', function(e) {
				...
				if(onlyMemory == "yes") {
					...
					$('#my-area-list button[data-role="save-dong"]').first().trigger('focus');
				} else {
			...
			$wrapper.on('click', 'ul[data-role="result-holder"] button', function(e) {
				var $item = $(this);
