...
(function($, window, document){
	...
	function showFFLocationChk(geojson) {
		if(player && player.vmap) {
			player.vmap.removeLocation();
			var geojsonObj = JSON.parse(geojson);
			var list = player.vmap.readGeojson(geojsonObj);
			for(var i in list) {
				//addSpecialPoint(coord, imageSrc, imageScale, options)
				player.vmap.addSpecialPointChk(list[i].coord, null, null, { title: list[i].data["name"] });
			}
		}
	}
			...
			success: function(data) {
				if(data && data.length > 0) {
					...
					for(var i = 0 ; i < data.length && i < 6 ; i++) {
						...
						tr += '	<td><span>' + awsItem.awsWss + '</span></td>';
	...
	-레이더 : 강수
	-위성 : RGB천연
	-산불영상 : 산불탐지, 산불방사열에너지, 산불열원강조
	-관측차량
	*/
	function updateSfc() {
		if(player) {
			player.kmapLayerUpdate([ { name:"sfc", options:{  } } ]);
			player.createKmap(player.options.kmapOptions);
		}
	}
	...
	function updateSat() {
		if(player) {
			player.kmapLayerUpdate([ { name:"sat", options:{ satIndex: 10 } } ]);
		}
	}
	function updateFFSat() {
		if(player) {
			player.kmapLayerUpdate([ { name:"sat", options:{ satIndex: 14 } } ]);
		}
	}
	function updateFFSatImg() {
		if(player) {
	...
	function onKmapOpts() {
		if(player) {
			if(!player.options.kmapEnabled) {
				player.options.type = "kmap";
				player.options.kmapEnabled = true;
			}
		}
	}
	function offKmapOpts() {
		if(player) {
			if(player.options.kmapEnabled) {
				player.options.type = "sat";
	...
	var selections = [
		[ '관측', [ { name:'지역별상세관측', update: updateSfc } ]],
		...
		[ '위성 산불영상', [ { name: '산불탐지', update: updateFFSatImg }, { name: '산불방사열에너지', update: updateFrpSatImg }, { name: '산불열원강조', update: updateFfeSatImg } ]],
	...
	function updateDataSelections(eventSource) {
		...
		for(var i in selection[1]) {
			var s = selection[1][i];
			if(s.name == '지역별상세관측') {
				$obsBox.append('<li><a href="javascript:void(0);" onclick="window.open(\'https://www.weather.go.kr/plus/land/current/aws_graph_popup.jsp\')" data-role="data-toggle" data-obs="' + i + '" ' + (i == 0 ? 'class="on"' : '') + '>' + s.name+ '</a></li>');
			}
			else {
	...
	function executeDataToggle(obs, eventSource) {
		...
		if(data && data.update) {
			if(eventSource) {
				if(selectedIndex != 6) data.update();
			} else {
				data.update();
			}
		}
	...
	function addEventHandler() {
		...
		*/
		...
		$('#btn-change-type').on('click', function(e) {
			...
			if(typeof ffLocationGeoJson !== "undefined" && ffLocationGeoJson) {
				showFFLocationChk(ffLocationGeoJson);
			}
		});
