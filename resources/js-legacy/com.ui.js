...
//	weatherUI.slickNumTab('.silck-ni-wrap');
	weatherUI.slickAroNonLoop('.map-cont-slick')
	weatherUI.slickBanner('.intro-slick' , '.intro-btn-wrap')
	// weatherUI.swiperScroll('.scroll-js');
	// weatherUI.swiperTabScroll('.weather-info-list');
}
	...
	$(obj).on('click', function(e) {
		...
		if(targetId == "movi-toggle") {
			if($(this).hasClass('off')) {
				...
				if(appConfig && appConfig.config && appConfig.config.state && appConfig.config.state) {
					appConfig.config.state.uiOptionShow = true;
				...
				if(appConfig && appConfig.config && appConfig.config.state && appConfig.config.state) {
					appConfig.config.state.uiOptionShow = false;
					appConfig.writeConfig();
				}
			}
		} else {
...
//			$('.maintab').find('button').eq(nextSlide).addClass('on');
		...
		if(win_W > 1101){
			...
			$(window).scroll(function(){
				if((contH - $(window).height()) > $(window).scrollTop()){
					...
					$('.subp header').css({'top':contH - $(window).height() , 'position' : 'absolute'});
					$('.mainp header').css('top',0);
				};
			})
		}else{
	...
	accordion: function(obj){
		...
		function event(){
			...
			if(accordionBtn.addClass('acc-key-press')) {
				accordionBtn.on('keypress',clickEvent);
			}
			accordionBtn.on('click',clickEvent);
		};
...
// scrollYscroll-js
	...
	scrollSlide : function(obj, com){
		...
		function change(){
			...
			if(winW  > 1100){
				scrollArea.css('width',scrollT + 100);
