...
// var SEARCH_LIMIT = 100; // 최대 검색 범위
SearchManager.prototype.searchOptions = [];
SearchManager.prototype.SEARCH_LIMIT = 3;
SearchManager.prototype.SEARCH_SIZE = 5;
...
var toggleOption = function(option) {
  ...
  if (option.toggle == false) {
    // true로 전환이 되기전에 체크한다.
    var limit = SearchManager.prototype.SEARCH_LIMIT;
...
//    console.log("SearchManager.initFindOption isUndefined(lonlat_search).");
  ...
  for (var i = 0 ; i < options.length ; i++) {
    if (optionManager.isBeginLonlat(options[i], lonlat_search)) {
      return options[i];
    }
  }
  return undefined;
