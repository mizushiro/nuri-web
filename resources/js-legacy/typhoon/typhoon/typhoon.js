...
var mapManager = {
	...
	extents : {
		...
		*/
		...
		// GK2A(천리안위성(2호기) 영상 위치 및 좌표
		base : ol.proj.transformExtent([-40, -40, 180, 60], "EPSG:4326", "EPSG:111111"),	//적도0~북위60도 경도 100~180 [100, 0, 180, 60]
		satellite : [-4396000, -4396000, 6796000, 4396000]									//위성 영상의 표출범위 
...
mapManager.setSateliteSources = function() {
	...
	function setSatImg(list) {
		console.log("function setSatImg(list) :: Start");

		var image = list[list.length-1];
		var s = new ol.source.ImageStatic({  
			url: image.fileName,						// 위성 영상 경로
			projection: mapManager.projection,			// 위성 영상 프로젝션
			imageExtent: mapManager.extents.satellite	// 위성 영상 좌표
		});  
	...
	function loadImage(list) {
		console.log("function loadImage(list) :: Start");
		var loadedCnt = list.length;
		if(loadedCnt > 0) {
			for(var i = 0; i < list.length ; i++) {
				console.log("Satellite Image :: " + list[i].fileName);
				
				imageObj.attr('src', list[i].fileName); 
	...
	/**
	 * 지금은 현재 진행중인 태풍 또는 TD가 있을 경우에만 위성영상 표출..
	 */
	if(key.length < 1) {
		
		console.log("현재 진행 중인 태풍 또는 TD가 없는 경우");
		// 현재 진행 중인 태풍 또는 TD가 없는 경우..(최근 영상 8건 조회 : tm=2015.07.11.22&x=29&y=13&timeTerm=1)
		$.get(urlPrefix + '/renew2021/rest/typhoon/satelliteImageList.do', function(list) {
			if(list) {
				for(var i = 0 ; i < list.length ; i++) {
					if(list[i].fileName.substring(0,4) != 'http') {
						list[i].fileName = urlPrefix + list[i].fileName;
					}
				}
		...
		for(var i in centers) {
			...
			if(info.tmTp.indexOf("F") < 0){//과거~현재 	
				...
				console.log("통보일시 info.tm :: " + info.tm);
			} 
		}
		
		// 현재 진행 중인 태풍 또는 TD가 있는 경우..
		// 통보일시에 해당하는 위성영상을 표출한다.
		if(list.length > 0) {
			
			console.log("현재 진행 중인 태풍 또는 TD가 있는 경우");
			
			$.get(urlPrefix + '/renew2021/rest/typhoon/satelliteImageListForItem.do', {'tms' : list}) 
				.done(function(r) {
					console.log("Satellite Image Search Success :: " + r);
					if(r) {
						for(var i = 0 ; i < r.length ; i++) {
							r[i].center = centers[r[i].centerIndex];
							if(r[i].fileName.substring(0,4) != 'http') {
								r[i].fileName = urlPrefix + r[i].fileName;
							}
							console.log("r[i].center :: " + r[i].center);
						}
				...
				.fail(function(e) {
					console.log("Satellite Image Search Failed :: " + e);
					mapManager.removeControl(loadingBarControl);
				});
...
mapManager.TapControl = function(opt_options) {
	...
	var html = '<div class="tab list jx">'			 
				+'<ul>'
				+'<li class="active" id="basic_id">'
				...
				+'</li>'
				+'</ul>'
				+'<input type="hidden" id="pre_button" name="pre_button" value="basic_id">'
				+'</div>';
	  ...
	  var handleTapControl = function(e) {
			 ...
			 * 범례 이미지 추가 (기본)
			 */
			$("#legendImg").attr("src",urlPrefix + "resources/image/typoon/typ_legend.png");
			...
			/**
			 * "기본화면", "위성영상", "최근접예상", "위험영역" 선택에 따른 이벤트 처리
			 * basic_id = 0, statistics_id = 1, approach_id = 2, danger_id = 3
			 */
			switch(idx) {
				case 0 : //기본화면 (GIS 지도)
				...
				case 3 : // 위험영역
					...
					//범례 이미지 추가 (위험반경) 예정
					$("#legendImg").attr("src", urlPrefix + "resources/image/typoon/typ_legend_03.png");
	...
	this.map = new ol.Map({
		...
		*/
		...
		view: new ol.View({
			...
			extent : ol.proj.transformExtent([-40, -40, 180, 60], "EPSG:4326", "EPSG:111111"),	//적도0~북위60도 경도 100~180 [-40, -40, 180, 60] - minx,miny,maxx,maxy
			extent : ol.proj.transformExtent([113, 15, 150, 46], "EPSG:4326", "EPSG:111111"),	//적도0~북위60도 경도 100~180 [-40, -40, 180, 60] - minx,miny,maxx,maxy
			extent : ol.proj.transformExtent([100, 0, 160, 50], "EPSG:4326", "EPSG:111111"),	//적도0~북위60도 경도 100~180 [-40, -40, 180, 60] - minx,miny,maxx,maxy
			*/
...
mapManager.addEventSelectBox = function(){
	...
	$('button[data-role="select-item"]').on('click', function(e){
		...
		//선택된 옵션 색깔 표시
		$selectbox.find("option").removeClass("selected"); 
		...
		if(item instanceof ContentManager.Typhoon) {
			...
			graphManager.showRadTd(item, graphId+"5", 1 , 1);   //20210503 상세정보 TD 그래프 '반경' 추가
			graphManager.showSpTd(item, graphId+"6", 1 , 1);
			
			//2.graph detail set
			graphManager.showWsTd(item, graphId+"Popup4", 3, 2);
			graphManager.showRadTd(item, graphId+"Popup5", 3, 2);   //20210503 상세정보 TD 그래프 '반경' 추가
			graphManager.showSpTd(item, graphId+"Popup6", 3, 2);
...
//									tmComments ="";
//								}		
								if(info.tmTp.indexOf("F") > -1){  //20210506 TD에서 태풍예상되는 경우 (미래), 손태림
...
//								}								
						...
						var contents = typhoonLang.msg('typho.0014'); //<li>중심위치:{0}˚N,{1}˚E</li><li>최대풍속(중심기압):{2}㎧({3}hPa){4}</li><li>강풍반경(예외반경):{5}㎞({6} 약{7}㎞){8}</li>
							contents = contents.format(
													...
													typhoonUtils.transValToStrengthTdLow(info.ws, info.tp), //20200508 손태림 typhoonUtils.transValToStrength(info.ws),  //wind speed
													...
													typhoonUtils.zeroToBlank(info.rad25),
													typhoonUtils.transValToDir(info.ed25),
													typhoonUtils.zeroToBlank(info.er25)
										);
							
						if(typhoonUtils.transValToDir(info.ed15)=="" || typhoonUtils.zeroToBlank(info.er15,'-')=='-') {  // 강풍반경의 예외방향이나 예외반경이 없을 경우 
							if(typhoonUtils.transValToDir(info.ed25)=="" || typhoonUtils.zeroToBlank(info.er25,'-')=='-') { // 폭풍반경의 예외방향이나 예외반경이 없을 경우
						   ...
						   else { // 폭풍반경의 예외방향이나 예외반경이 있을 경우
								var contents = typhoonLang.msg('typho.0014.02'); //<li>중심위치:{0}˚N,{1}˚E</li><li>최대풍속(중심기압):{2}㎧({3}hPa){4}</li><li>강풍반경(예외반경):{5}㎞({6} 약{7}㎞){8}</li>
								contents = contents.format( 
										Math.round(info.lat * 10)/10,
										Math.round(info.lon * 10)/10,
										typhoonUtils.zeroToBlank(info.ws,'-'),
						...
						else { // 강풍반경의 예외방향이나 예외반경이 있을 경우
							if(typhoonUtils.transValToDir(info.ed25)=="" || typhoonUtils.zeroToBlank(info.er25,'-')=='-') { // 폭풍반경의 예외방향이나 예외반경이 없을 경우
								var contents = typhoonLang.msg('typho.0014.03'); //<li>중심위치:{0}˚N,{1}˚E</li><li>최대풍속(중심기압):{2}㎧({3}hPa){4}</li><li>강풍반경(예외반경):{5}㎞({6} 약{7}㎞){8}</li>
								contents = contents.format( 
										Math.round(info.lat * 10)/10.0,
										Math.round(info.lon * 10)/10.0,
										typhoonUtils.zeroToBlank(info.ws,'-'),
						...
						popupObj.find("span.arrow > img").attr("src", urlPrefix + "resources/image/typoon/typ_new/bg_arrow_map_" + className + ".png"); 
						if(typeof popup !== "undefined"){ 
						...
						if(typhoonLang.currentLang === "en"){
							...
							//"제 {0}호 열대저압부 {1}<br/>({2})"
							if(info.rTp === "TD"){
								if(info.tmTp.indexOf("P") > -1){ //20210506 과거의 경우 이전 호가 나오도록, 손태림 
									title = typhoonLang.msg('typho.0015').format(info.ref.year, info.rSeq, tmComments, tmString);  
								}
								else{
									title = typhoonLang.msg('typho.0015').format(info.ref.year, info.ref.no, tmComments, tmString);  //분석
								}
							}else{
								title = typhoonLang.msg('typho.0015').format(info.ref.year, info.ref.no, tmComments, tmString);  //분석
							}
...
/*						var contents = typhoonLang.msg('typho.0016').format(Math.round(info.lat * 10) / 10,
																			...
																			typhoonUtils.zeroToBlank(info.dir, '-'));*/ //20210504 상세정보 TD툴팁 TY로 나오기
...
mapManager.LoadingBarControl = function(opt_options) {   
	 ...
	 $("#" + mapManager.config.selectbox).prop('disabled', true); 
	  var html = '<p><img src="' + urlPrefix + 'resources/image/typoon/typ_new/loading_bar.gif" width="339px" height="26px" alt="로딩바" /></p>'; 
  	  var options = opt_options || {};
	  var obj = $(html); 
...
/** 
 * 현재 진행중인 태풍 또는 열대저압부가 없을 경우 화면 출력
 *     - 현재 진행중인 열대저압부 또는 태풍이 없습니다.
 */
mapManager.addNoItemMsgControl = function(){
	...
	$("#typhoon_map").html('<img src="' + urlPrefix + 'resources/image/typoon/img_nonetyphoon.gif" alt="현재 진행중인 태풍 또는 열대저압부가 없습니다.">');
