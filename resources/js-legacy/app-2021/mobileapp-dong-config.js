...
(function($, window, document){
	...
	DongConfig.prototype = {
		...
		updateView: function(type, data, noFocus) {
			...
			var template = '{{#data}}<li><div class="radio-wrap"><a href="#" class="addr-chk-btn" data-code="{{code}}" data-name="{{name}}" data-x="{{x}}" data-y="{{y}}" data-lat="{{lat}}" data-lon="{{lon}}" data-level="{{level}}">{{name}}</a></div></li>{{/data}}';
			...
			var upButton = '<li><div class="radio-wrap"><a href="#" class="addr-chk-btn" data-goto="' + (type==TYPE.CITY?TYPE.WIDE:(type==TYPE.DONG?TYPE.CITY:""))+ '">↑ 상위</a></li>';
			if(type == TYPE.WIDE) {
				$view.html(html);
			} else {
