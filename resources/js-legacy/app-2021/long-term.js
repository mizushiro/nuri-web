...
(function($, window, document){
	...
	LongTerm.prototype = {
		...
		updateView: function(data) {
			...
			if(!isIE()) {
				if(self.isCmt) {
					template = '<object tabindex="0" width="100%" height="1475px" data="' + self.getPathPrefix() + 'repositary/xml/fct/mon/{{cmtfilename}}#toolbar=0&navpanes=0&scrollbar=1&view=fix&messages=0" type="application/pdf">';
					...
					template += '<iframe src="' + self.getPathPrefix() + 'repositary/xml/fct/mon/{{cmtfilename}}#toolbar=0&navpanes=0&scrollbar=1&view=fix&messages=0" width="100%" height="100%" style="border:none;" title="' + self.type + '개월 전망">브라우저가 PDF 보기를 지원하지 않습니다. 다운로드하셔서 보시기 바랍니다.'; 
					template += '<a href="' + self.getPathPrefix() + 'repositary/xml/fct/mon/{{cmtfilename}}">다운로드 PDF</a>';
					template += '</iframe>';
					template += '</object>';
				} else {
					template = '<object tabindex="0" width="100%" height="1475px" data="' + self.getPathPrefix() + 'repositary/xml/fct/mon/{{filename}}#toolbar=0&navpanes=0&scrollbar=1&view=fix&messages=0" type="application/pdf">';
					...
					template += '<iframe src="' + self.getPathPrefix() + 'repositary/xml/fct/mon/{{filename}}#toolbar=0&navpanes=0&scrollbar=1&view=fix&messages=0" width="100%" height="100%" style="border:none;" title="' + self.type + '개월 전망">브라우저가 PDF 보기를 지원하지 않습니다. 다운로드하셔서 보시기 바랍니다.'; 
