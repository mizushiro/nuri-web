...
(function($, window, document){
	...
	MyAreaList.prototype = {
		...
		updateBookmarks: function() {
			...
			template += '<a class="dong" title="보기" href="#view-bookmark" data-role="view-bookmark" data-index={{index}} ';
			template += '{{#wide}}data-wide-code="{{code}}" data-wide-name="{{name}}"{{/wide}} ';
			template += '{{#city}}data-city-code="{{code}}" data-city-name="{{name}}"{{/city}} ';
			template += '{{#dong}}data-dong-code="{{code}}" data-dong-name="{{name}}" data-x="{{x}}" data-y="{{y}}" data-lat="{{lat}}" data-lon="{{lon}}">{{name}}</a>{{/dong}}';
		...
		onAreaShow: function(bookmark) {
			GlobalEvent.trigger($.Event("onAreaShow"), [bookmark, this]);
		},
		addEventHandler: function() {
			...
			$wrapper.on('click', 'a[data-role="view-bookmark"]', function(e) {
				e.preventDefault();
				var item = $(this);
				var index = parseInt(item.attr('data-index'));
				var bookmark = {
						...
						dong: { code: item.attr('data-dong-code'), name: item.attr('data-dong-name'), lat: item.attr('data-lat'), lon: item.attr('data-lon'), x: item.attr('data-x'), y: item.attr('data-y')},
						lat: item.attr('data-lat'), lon: item.attr('data-lon'), x: item.attr('data-x'), y: item.attr('data-y')
				};
				self.onAreaShow(bookmark);
				self.closeConfig();
