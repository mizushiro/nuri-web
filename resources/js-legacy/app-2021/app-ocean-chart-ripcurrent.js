/**
 *  바다 - 해양일기도 - 이안류 예측정보(url: /ocean/chart/ripcurrent)
 */
'use strict';
(function($, window, document){
    
    $('#ripcurrent-data-panel').on('click', '.day1-rip a[data-map-mode-action="open"]', function(e) {
		e.preventDefault();
    ...
    $('#ripcurrent-data-panel').on('click', '.day2-rip a[data-map-mode-action="open"]',function(e) {
		e.preventDefault();
		var isClosed = !$(this).hasClass('on');
		if(isClosed) {
			// open
    ...
    $('#ripcurrent-data-panel').on('click', '.day3-rip a[data-map-mode-action="open"]', function(e) {
		e.preventDefault();
		var isClosed = !$(this).hasClass('on');
		if(isClosed) {
			// open
	...
	$('.cmp-rip-chart .left-rip .wrap-rip-spot li').each(function(index,value) {
		$('.cmp-rip-chart .left-rip .wrap-rip-spot .rip-spot-' + (index+1) +' img').hover(function() {
			$('.cmp-rip-chart .left-rip .wrap-rip-spot .rip-spot-' + (index+1) +' span').css({"visibility":"visible"});
		}, function(){
			$('.cmp-rip-chart .left-rip .wrap-rip-spot .rip-spot-' + (index+1) +' span').css({"visibility":"hidden"});
		});
	});
	...
	function initMap() {
					...
					var markerVectorLayer = new ol.layer.Vector({
						...
						style: new ol.style.Style({
							image: new ol.style.Icon({
								anchor: [0.5,1],
								src: `${window.appBase}/resources/image/marine/ico_rip_${s.todayLevel}.png`,
								scale: 0.4,
							})
						})
					});
		...
		function updateData(station) {
			$.ajax({
				url: "ripcurrent-data.do",
				method: "get",
				data: { nameEn: station.nameEn },
				success: function(html) {
					$('#ripcurrent-data-panel').html(html);
				},
				error: function(err) {
					alert('오류가 발생하였습니다.');
				}
