...
(function($, window, document){
	...
	SeaBuoy.prototype = {
		...
		requestBuoyData: function(data) {
			...
			return $.ajax({
				url: prefix + "weather/ocean/buoy-obs-data.do",
				data: data,
				dataType: "json"
			});
		},
		...
		updateBuoyList: function(data) {
			...
			if(data.isSuccess) {
				...
				bodyHtml += "<li><a href='/w/weather/ocean/buoy.do' class='cmp-blue-btn' target='_blank' title='새창열림'>";
		...
		createTempChart: function(chartId, airTempData, waterTempData) {
			console.log('createTempChart', airTempData, waterTempData);
		...
		createPaHmChart: function(chartId, paData, hmData) {
			console.log('createPaHmChart', paData, hmData);
			var minValue = 9999;
			var maxValue = -9999;
		...
		createWindChart: function(chartId, windData, gustData) {
			console.log('createWindChart', windData, gustData);
		...
		createWaveChart: function(chartId, maxWaveData, waveData, wpData) {
			console.log('createWaveChart', maxWaveData, waveData, wpData);
			var minValue = 9999;
			var maxValue = -9999;
		...
		addEventHandler: function() {
			...
			$wrapper.on('click', 'button[data-role="select-ocean"]', function(e) {
				console.log('select ocean triggered')
