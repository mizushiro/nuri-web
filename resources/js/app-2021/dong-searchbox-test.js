...
(function($, window, document){
	...
	DongSearchbox.prototype = {
		...
		searchPlaces: function(query, page, source) {
			...
			var category = $('#index-local-category').val();
			self.places(query, page, category).then(function(data) {
		...
		createPlaces: function() {
			...
			if(data && data.length > 0) {
				var template = '{{#data}}<li class="place"><a href="#" data-role="place" data-address="{{address}}" data-road-address="{{roadAddress}}" data-longitude="{{longitude}}" data-latitude="{{latitude}}" data-x="{{x}}" data-y="{{y}}" data-dong-code="{{dongCode}}"><strong>{{title}}</strong><span>{{address}}</span></a></li>{{/data}}';
		...
		places: function(query, page, category) {
			return $.ajax({
				url: RENEW_PREFIX + "rest/main/place-search.do",
				data: {query: query, start: page, src: 'A4', category: category},
				dataType: "json"
			});
		}
	};
