...
//		weatherUI.scrollY('.scroll-js');
	...
	window.udpateWeatherQList  = function(data) {
		...
		if(data.pageNumber > pagePerBlock) {
			pagination += '<button class="page-prev" title="이전페이지" data-page="' + (pageStart-pagePerBlock) + '" tabindex="0">&nbsp;</button>';
		} else {
			pagination += '<button class="page-prev" title="이전페이지" style="visibility: hidden">&nbsp;</button>';
		}
		...
		for(var i = 0; i < pagePerBlock ; i++) {
			...
			if(pageNum == data.pageNumber) {
				pagination += '<button class="num on" title="선택됨" data-page="' + pageNum + '" tabindex="0">' + pageNum + '</button>';
			} else {
				pagination += '<button class="num" data-page="' + pageNum + '" tabindex="0">' + pageNum + '</button>';
			}
		}
		if(pageStart + pagePerBlock -1 < totalPage) {
			pagination += '<button class="page-next" title="다음페이지" data-page="' + (pageStart+pagePerBlock) + '" tabindex="0">&nbsp;</button>';
		} else {
			pagination += '<button class="page-next" title="다음페이지" style="visibility: hidden">&nbsp;</button>';
		}
		...
		$('.page-num.page').html(pagination).unbind('click').on('click','.num, .page-prev, .page-next',function(e) {
			...
			addScript('https://www.kma.go.kr/kma/wtouch/wtouchQListJs.jsp?page=' + toPage);
		});
	}
	window.udpateWeatherSList  = function(data) {
		...
		$('.page-num.page').html(pagination).unbind('click').on('click','.num, .page-prev, .page-next',function(e) {
			var toPage = $(this).attr('data-page');
			showLoading('weather-on-list', true, 'light');
			addScript('https://www.kma.go.kr/kma/wtouch/wtouchSListJs.jsp?page=' + toPage);
		});
	}
