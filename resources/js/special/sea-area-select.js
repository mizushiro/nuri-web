...
(function($, window, document){
	...
	MAX_SHOW_HISTORY = 6,
	SeaAreaSelect = function(wrapperId) {
	...
	SeaAreaSelect.prototype = {
		...
		updateView: function(type, data) {
			...
			var template = '{{#data}}<li><div class="radio-wrap"><a href="#" id="rdo_sea_config_{{type}}{{index}}" class="addr-chk-btn rdo-g {{#checked}}on{{/checked}}" name="{{type}}Area" data-value="{{code}}" data-name="{{name}}">{{name}}</a></div></li>{{/data}}';
		...
		updateRecent: function() {
			...
			$view.html(html);
			var $title = $wrapper.find('a[data-role="recent-selected"]');
		...
		addEventHandler: function() {
			var base = this;
			var $wrapper = $('#' + base.id);
			$wrapper.on('click', 'div[data-role="bookmark-holder"] a', function(e) {
				e.preventDefault();
			...
			$wrapper.on('click', 'div[data-role="recent-holder"] a', function(e) {
				e.preventDefault();
				var bookmark = readBookmarkAttribute(this);
			...
			$wrapper.find('a[data-role="save-area"]').on('click', function(e) {
				e.preventDefault();
				...
				if(newBookmark) {
					...
					$wrapper.find('a[data-role="show-favorite"]').trigger('click');
				}
			});
			$wrapper.find('a[data-role="view-area"]').on('click', function(e) {
				e.preventDefault();
				...
				if(newRecent) {
					...
					$wrapper.find('a[data-role="show-recent"]').trigger('click');
				}
			});
			
			$wrapper.find('a[data-role="show-favorite"]').on('click', function(e) {
				e.preventDefault();
				base.onSeaBookmarkShow();
				base.closeConfig();
			});
			$wrapper.find('a[data-role="show-recent"]').on('click', function(e) {
				e.preventDefault();
			...
			// 최상위 바다예보구역 클릭
			$wrapper.on('click', 'a[name="topArea"]', function(e) {
				e.preventDefault();
				$wrapper.find('a[name="topArea"]').removeClass('on');
				$(this).addClass('on');
			...
			// 중간 바다예보구역 클릭
			$wrapper.on('click', 'a[name="midArea"]', function(e) {
				e.preventDefault();
				$wrapper.find('a[name="midArea"]').removeClass('on');
				$(this).addClass('on');
				var code = $(this).attr('data-value');
				var name = $(this).attr('data-name');
			...
			// 중간 바다예보구역 클릭
			$wrapper.on('click', 'a[name="btmArea"]', function(e) {
				e.preventDefault();
				$wrapper.find('a[name="btmArea"]').removeClass('on');
				$(this).addClass('on');
				var code = $(this).attr('data-value');
				var name = $(this).attr('data-name');
	...
	if (typeof exports !== 'undefined') exports.SeaAreaSelect = SeaAreaSelect;
	else window.SeaAreaSelect = SeaAreaSelect;
