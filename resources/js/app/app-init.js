...
//main left tab
$('ul.tab[data-role="global-left-tab"]').on('click', 'button', function(e) {
	var tabNum = $(this).attr('data-tab');
	if(tabNum == "2") {
		// 기상+
		var $header = $(this).parents("header"); 
		$header.find('.left-menu').addClass('wplus').find('.wplus-menu').show();
		$header.find('.wnuri-menu').hide();
		$('ul.tab[data-role="global-left-tab"] button').removeClass('on').attr('title','');
		if(!$(this).hasClass('on')) $(this).addClass('on').attr('title','선택됨');
	} else if(tabNum == "1") {
		// 날씨
		var $header = $(this).parents("header"); 
...
GlobalEvent.on('onAreaBookmarkShow', function(e, bookmark, sender) {
	var mainURL = window.appBase;
	var path = window.location.pathname.split(/\?/)[0];
	if(path != mainURL) {
		// 첫화면이 아닐 경우 
		window.location.href = window.appBase + "weather/today.do";
	} else {
		// 첫화면일 경우 날씨탭을 오픈
		var tab = $('.maintab > div > button[data-main="2"]');
		tab.trigger('click');
	}
});
GlobalEvent.on('onAreaRecentShow', function(e, bookmark, sender) {
	var mainURL = window.appBase;
	var path = window.location.pathname.split(/\?/)[0];
	if(path != mainURL) {
		// 첫화면이 아닐 경우 
		window.location.href = window.appBase + "weather/today.do#last-recent";
	} else {
		// 첫화면일 경우 날씨탭을 오픈
		var tab = $('.maintab > div > button[data-main="2"]');
		tab.trigger('click');
...
GlobalEvent.on('onAddSeaRecent', function(e, seaBookmerk, sender) {
	var oceanTodayURL = window.appBase + "ocean/today.do";
	var path = window.location.pathname.split(/\?/)[0];
	if(path != oceanTodayURL) {
		// 오늘의 바다가 아닐 경우
		window.setTimeout(function() {
			window.location.href = window.appBase + "ocean/today.do#last-recent";	
		},10);
	}
});
GlobalEvent.on('onSeaRecentShow', function(e, seaBookmerk, sender) {
	var oceanTodayURL = window.appBase + "ocean/today.do";
	var path = window.location.pathname.split(/\?/)[0];
	if(path != oceanTodayURL) {
		// 오늘의 바다가 아닐 경우
...
GlobalEvent.on('onSeaBookmarkShow', function(e, seaBookmerk, sender) {
	var oceanTodayURL = window.appBase + "ocean/today.do";
	var path = window.location.pathname.split(/\?/)[0];
	if(path != oceanTodayURL) {
		// 오늘의 바다가 아닐 경우
		window.setTimeout(function() {
			window.location.href = window.appBase + "ocean/today.do";	
		},10);
	} 
});
