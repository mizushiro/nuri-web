...
(function($, window, document){
	...
	VshortShort = function(wrapperId, caller) {
	...
	VshortShort.prototype = {
		...
		getData: function(x, y, unit) {
			var base = this;
			var $wrapper = $('#' + base.id);
			var prefix = this.getPrefix();
			var theme = $wrapper.hasClass("light-theme")?"Light":"Dark";
			var data = {
				caller: this.caller,
				x: x,
				y: y,
			...
			return $.ajax({
				url: prefix + "wnuri-fct/main/vshort-short.do",
				data: data,
				dataType: "html"
			});
		},
		...
		requestRemote: function() {
			...
			return base.getData(base.state.selectedBookmark.dong.x, base.state.selectedBookmark.dong.y, base.state.unit);	
		},
		...
		updateView: function(html) {
			...
			var $wrapper = $('#' + base.id);
			$wrapper.html(html);
			weatherUI.scrollSlide('#' + base.id + ' .weather-info-list' , '#' + base.id + ' .swiper-pagination');
			weatherUI.slickChange4('#' + base.id + ' .weather-list');
			$('.main-typhoon .point-address').html(base.state.selectedBookmark.fullName);
		},
		error: function() {
			var base = this;
	...
	if (typeof exports !== 'undefined') exports.VshortShort = VshortShort;
	else window.VshortShort = VshortShort;
