...
typhoonUtils.getIntensity  = function(){
	...
	return {
		...
		SUPER_STRONG : typhoonLang.msg('utils.0026'),  //20200508 손태림
		NONE : typhoonLang.msg('utils.0017')
	};
};
...
typhoonUtils.transValToStrength = function (val){ 
	...
	*/ 
	...
	}else if(val>=44 && val < 54){	
		str = intensity.VERY_STRONG; //매우강
	}else if(val>=54){	
		str = intensity.SUPER_STRONG; //초강력  20200508 손태림
	}else{
		str = intensity.NONE;
	}
...
typhoonUtils.zeroToBlank = function (val, defaultVal){   //강풍반경, 폭풍반경
