			...
			active : { 
				...
				startTd : new ol.style.Style({
					image: new ol.style.Icon({
                        scale: 1,  
                        opacity: 1,
                        src: '../../images/weather/typoon/gis/start_td.png'
                    }) 
				}),  
		...
		,icon2 : {	//예보 태풍중심 아이콘
			...
			forecastWeak : new ol.style.Style({
				image: new ol.style.Icon({
                    scale: 1,  
                    opacity: 1,
                    src: '../../images/weather/typoon/gis/weak_red.png'
                })
			}),
		...
		for(var i in item.centers){
			...
			if(info.tmTp.indexOf("P")<0){	//현재 부터 표출 
				...
				body += trFormat.format(timeStr
									...
									,typhoonUtils.zeroToBlank(rad15Msg)
									,typhoonUtils.transValToStrengthTdLow(info.ws, info.tp)
...
ContentManager.getCenterIcon = function(info, isActive){
	...
	function getTdStyle(info, isActive){
		...
		if(info.tmTp == "C" &&  info.ref.tmEnd === "Y" ){ //미래 이거나 TD예보종료 지점인 경우
			...
			/*
			console.log("info.tmTp=="+info.tmTp);
			console.log("info.tmTp=="+info.tmTp);
			console.log("info.ws=="+info.ws);
			console.log("info.ws=="+info.ws);
			*/
	...
	function getTypStyle(info, isActive){
		...
		if(info.tp == "LOW"){ // L(온대저기압) 현재 또는 미래에는 빨간 아이콘으로
			...
			/*
			console.log("==========================");
			console.log("info.ft15=="+info.ft15);
			console.log("info.tmTp=="+info.tmTp);
			console.log("info.ws=="+info.ws);
			console.log("==========================");
			*/
...
ContentManager.prototype.loadTlb = function(){
	...
	function loadTlb2(data63Arr){
		...
		//현재진행중인 TD 목록 조회
//		$.post('../../DAOU/TOS/RTKO64/RTKO64.tbl', function(data64) {
	...
	//현재진행중인 태풍 목록 조회 /ko/weather/typh
//	$.post('../../DAOU/TOS/RTKO63/RTKO63.tbl', function(data63) {
...
ContentManager.prototype.loadTyphoon = function(osp, ini){
	...
	$.get(osp, function(data) {//load OSP file (track/forecast/centers)  
		...
		for(var i = 1 ; i<dataArr.length;i++){
			...
			/**
			 * tempArr[] : [ 201902190900,4.8,160.6,괌 동남동쪽 약 1980 km 부근 해상,W,41,1004,15,TD,0,,0,0,,0,0,2019,1,TD ]
			 *  .
			 *  .
			 *  .
			 */
...
//					}
			...
			var dirName = typhoonUtils.transValToDir(tempArr[4]);//W->서
...
//			   	} 
		...
		/**
		 * centerArr[0].getGeometry().getCoordinates() : 3841402.6736558136,-2123717.8231987283
		 */
		...
		$.get(ini, function(doc) {   
			   ...
			   var infoArr = docArr[0].split(',');//201902201500,2,1
	...
	}).fail(function(xhr, status, errorThrown) { // alert("진입 실패 : " + status + ", " + errorThrown);
		contentManager.appendSelectboxOption(undefined);
	});
}; 
		...
		for(var i = 1 ; i<dataArr.length;i++){
			...
			if( typeof nextTempArr !== "undefined" && checkValidationRow(nextTempArr)){
				...
				if(tempArr[18].length<1 && nextTempArr[18].length>0
					...
					|| tempArr[18].length>0 && nextTempArr[18].length>0 && tempArr[18]!==nextTempArr[18]){
			...
			if(rTyphoonName.length > 0 && centerArr.length > 0){
				...
				if(info.rTp === "TY"){
					...
					if(perInfo.rYear === currentTmDate.getFullYear().toString()){
						info.rDesc = typhoonLang.msg('conte.0010').format(perInfo.rSeq,rTyphoonName);
					}else{
						info.rDesc = typhoonLang.msg('conte.0013').format(perInfo.rYear,perInfo.rSeq,rTyphoonName);
					}
				}
			}
			if(isCut){
...
//				if(currentYear!=typhoon.year){
					optionText = typhoonLang.currentLang === "en" ? typhoonLang.msg('conte.0016').format(typhoon.year, typhoon.no, typhoon.enName)
							:  typhoonLang.msg('conte.0016').format(typhoon.year, typhoon.no, typhoon.name, typhoon.enName);
//				}else{
//					optionText = typhoonLang.currentLang === "en" ? typhoonLang.msg('conte.0014').format(typhoon.no, typhoon.enName)
//							:  typhoonLang.msg('conte.0014').format(typhoon.no, typhoon.name, typhoon.enName);
//				}
...
//	var forecastSplength = coordinates.length;
//	for(var i in this.centers){
//		coordinates.push(this.centers[i].getGeometry().getLastCoordinate());
//	} 
	var posArr = [];  
...
//		if(forecastSplength < minIdx){
//			var centerTm =this.centers[minIdx-forecastSplength].get("info").tm.toDate();
//			var mm = (centerTm.getMonth()+1).toString(); // getMonth() is zero-based
//			var dd = centerTm.getDate().toString();
//			var hh = centerTm.getHours().toString();
//			var ds = Math.round(minDistance/10)*10;
//			overlaytext = overlaytextFormat.format(city,mm,dd,hh,ds+"");
//		}
...
ContentManager.prototype.loadDangerInit = function(typhoon){ 
	 ...
	 * 위험반경 경로 변경
	 */
	$.post('../../repositary/xml/typ/raw/DAOU/DANGER/DANGER.tbl', function(data) { 
		var dataArr = data.split("\n");
...
ContentManager.prototype.loadDanger = function(typhoon, ini){ 
	...
	$.get(ini, function(doc){ 
		...
		// alert("진입 실패 : " + status + ", " + errorThrown);
	});     
};
