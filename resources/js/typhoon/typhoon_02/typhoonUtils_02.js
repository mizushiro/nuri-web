...
typhoonUtils.getIntensity = function() {
  return {
    ...
    , SUPER_STRONG: typhoonLang.msg('utils.0026')
    , NONE: typhoonLang.msg('utils.0017')
  };
};
...
typhoonUtils.transValToStrength = function(val) {
	...
	*/ 
	...
	if(val>=17 && val<25){
		...
		str = intensity.SUPER_STRONG; //초강력
	}else{
		str = intensity.NONE;
	}
...
//TD여부
typhoonUtils.isTyphoon = function(ws) {
  if (objectUtils.isUndefined(ws)) {
    return;
  }
  if (ws < 17) {
    return true;
...
typhoonUtils.getDistance = function(pos1, pos2) {
  ...
  var a = Math.sin(dlat / 2.0) * Math.sin(dlat / 2.0) + Math.cos(lat1 * DEGRAD) * Math.cos(lat2 * DEGRAD) * Math.sin(
...
typhoonUtils.areaToWindPoint = function(pos1, pos2) {
  ...
  if (((1 / Math.tan(lon * Math.PI / 180) - (nowLong - lon) * Math.PI / 180) * Math.sin(Math.sin(lon * Math.PI / 180) *
      (nowLat - lat + 0.0000000001) * Math.PI / 180)) >= 0) {
    wind = 90 - Math.atan(((1 / Math.tan(lon * Math.PI / 180) - (1 / Math.tan(lon * Math.PI / 180) - (nowLong - lon) *
      Math.PI / 180) * Math.cos(Math.sin(lon * Math.PI / 180) * (nowLat - lat) * Math.PI / 180))) / ((1 / Math.tan(
      lon * Math.PI / 180) - (nowLong - lon) * Math.PI / 180) * Math.sin(Math.sin(lon * Math.PI / 180) * (
      nowLat - lat + 0.0000000001) * Math.PI / 180))) * 180 / Math.PI;
  }
  else {
    wind = 270 - Math.atan(((1 / Math.tan(lon * Math.PI / 180) - (1 / Math.tan(lon * Math.PI / 180) - (nowLong - lon) *
      Math.PI / 180) * Math.cos(Math.sin(lon * Math.PI / 180) * (nowLat - lat) * Math.PI / 180))) / ((1 / Math.tan(
      lon * Math.PI / 180) - (nowLong - lon) * Math.PI / 180) * Math.sin(Math.sin(lon * Math.PI / 180) * (
      nowLat - lat + 0.0000000001) * Math.PI / 180))) * 180 / Math.PI;
  }
 ...
 * 강도등급 수정 (날씨누리 태풍찾아보기)
 */
typhoonUtils.transValToStrength2 = function(val, date) {
  ...
  if (val >= 17 && val < 25) {
	...
	str = intensity.SUPER_STRONG; //초강력 태풍찾아보기 20210429 손태림
  }
  else{
    str = intensity.NONE;
  }
